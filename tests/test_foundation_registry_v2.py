"""Repo-level regression tests for neuros-foundation v2.1 interoperability APIs."""
...
def test_foundation_catalog_spans_modern_modalities_and_industry() -> None:
...
def test_foundation_registry_filters_eeg_and_recent_years() -> None:
    ids = {card.id for card in DEFAULT_REGISTRY.filter(modality="eeg", min_year=2025)}
    assert "zuna-1.1" in ids
    assert "reve-base" in ids
    assert "ndt3" not in ids


def test_catalog_only_foundation_model_fails_closed() -> None:
    assert not DEFAULT_REGISTRY.availability("emotiv-axon").available
    with pytest.raises(AdapterUnavailableError):
        DEFAULT_REGISTRY.adapter("emotiv-axon")


def test_foundation_representation_probes_are_deterministic() -> None:
...
def test_foundation_linear_probe_finds_simple_signal() -> None:
...
def test_foundation_benchmark_records_protocol_and_similarity() -> None:
...
def test_sample_level_foundation_protocol_requires_explicit_acknowledgement() -> None:
...
def test_foundation_embedding_decoder_implements_neuros_contract() -> None:
