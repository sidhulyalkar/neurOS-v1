...
class TemporalOrderInterventionRepresentation:
    ...
    def __init__(
        ...
        *,
        ...
        try:
            ...
            raise ValueError(
                "wrapped representation must declare a valid evaluation_scope"
            ) from exc
        if scope is not EvaluationScope.SEQUENCE_LOCAL:
            raise ValueError(
                "temporal-order intervention requires a sequence-local method"
            )
        ...
        if not isinstance(wrapped_id, str) or not wrapped_id.strip():
            raise ValueError(
                "wrapped representation must expose a nonblank method_id"
            )
        try:
            fit_regime = FitRegime(method.fit_regime)
        except (AttributeError, TypeError, ValueError) as exc:
            raise ValueError(
                "wrapped representation must declare a valid fit_regime"
            ) from exc
        ...
        if method_id == wrapped_id:
            raise ValueError(
                "intervention method_id must differ from wrapped method_id"
            )
        ...
        self._wrapped_method_id = wrapped_id

    def _rng_for(self, sequence_id: str) -> np.random.Generator:
        digest = hashlib.sha256(
            f"neuros.temporal_order.v1\0{self.seed}\0{sequence_id}".encode(
                "utf-8"
            )
        ).digest()
    ...
    def _permutation(self, n_rows: int, sequence_id: str) -> np.ndarray:
        ...
        if n_blocks < 2:
            raise ValueError(
                "block_size must leave at least two temporal blocks to permute"
            )
        ...
        blocks = [
            np.arange(
                ...
                min((block + 1) * self.block_size, n_rows),
                dtype=np.int64,
            )
            for block in order
        ]
        ...
        if (
            permutation.shape != (n_rows,)
            or np.unique(permutation).size != n_rows
        ):
            raise RuntimeError("internal temporal permutation is not bijective")
        if np.array_equal(permutation, np.arange(n_rows)):
            raise RuntimeError(
                "temporal intervention unexpectedly produced identity order"
            )
        return permutation
    ...
    def embed(
        ...
        intervened = SequenceBatch(
            sequences=tuple(
                ...
                for sequence, permutation in zip(
                    evaluation.sequences,
                    permutations,
                    strict=True,
                )
            ...
            metadata={
                **dict(evaluation.metadata),
                "temporal_order_intervention": (
                    "deterministic_block_permutation_v1"
                ),
        ...
        if embedding.method_id != self._wrapped_method_id:
            raise ValueError(
                "wrapped representation output changed its declared method_id"
            )
        if embedding.fit_regime is not self.fit_regime:
            raise ValueError(
                "wrapped representation output changed its declared fit_regime"
            )
        if embedding.sequence_ids != evaluation.sequence_ids:
            raise ValueError(
                "wrapped representation changed evaluation sequence identity"
            )
        if len(embedding.sequences) != len(permutations):
            raise ValueError(
                "wrapped representation changed evaluation sequence count"
            )
        ...
        for sequence_id, source, latent, permutation in zip(
            ...
            if latent.shape[0] != source.shape[0]:
                raise ValueError(
                    "wrapped representation changed evaluation timepoint count"
                )
