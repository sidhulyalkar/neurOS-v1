...
class TransportClaimDecision:
    ...
    def launch_permitted(self) -> bool:
        # A claim alone is deliberately insufficient to start a worker.
        return False
...
class LocalAtomicCreateStore:
    ...
    def create_if_absent(self, key: str, payload: bytes) -> bool:
        ...
        try:
            ...
            try:
                temporary.unlink()
            except FileNotFoundError:  # pragma: no cover - defensive cleanup guard
                pass
...
def acquire_attempt_claim(
    ...
    *,
    ...
    ``created=True`` means this owner won the immutable claim. Replay by that same
...
def begin_attempt_invocation(
    ...
    *,
    ...
    Only ``created=True`` grants launch permission. An existing identical marker,
    even for the same owner, returns ``created=False`` and therefore forbids a
    second launch. Scheduler-loss recovery is an explicit adjudication problem,
    not a timeout-based scientific retry.
    """
