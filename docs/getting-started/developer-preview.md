...
Console scripts are resolved from the **active interpreter environment's scripts directory**, not by resolving the Python executable symlink. This matters for virtual environments: a venv Python may point to a base interpreter while its installed `neuros`, `neuros-models`, and `neuros-arena` entry points correctly live inside the venv.
...
- active Python/platform identity, environment prefix, and scripts directory;
- installed distribution names, versions, locations, and direct-install metadata;
- whether any package was editable;
- resolved public console-script paths;
...
The report is initialized before package/script preflight. CI uploads the report with `always()` semantics, so a journey failure is itself retained as a diagnostic artifact rather than disappearing because the success path never reached artifact publication.
